...
def configColumnTags(ibeacon_csv):
    ...
    :return: list<string>
    """
    column_tags = ["referencePoint_tag", "direction"]
...
def sliceAndAverage(referencePoint_csv, class_tag, direction_tag, column_tags, samples_dataset):
    """
    将数据按设定的时间片分割并求平均的方法
    ...
    while i < rownum:
        ...
        macs_rssi[0] = class_tag
        macs_rssi[1] = direction_tag
...
def loadAllCsvFile2SampleSet(csv_rootdir, column_tags):
    ...
    dirs = os.listdir(csv_rootdir)  # 列出文件夹下所有的目录与文件
    for class_dir in dirs:
        class_base_path = os.path.join(csv_rootdir, class_dir)
        files = os.listdir(class_base_path)
        class_tag = class_dir
        print("class_tag = {}".format(class_tag))
        for file in files:
            print("--加载csv文件{}到SampleSet:".format(file))
            file_path = os.path.join(class_base_path, file)
            # 加载 参考点的csv文件 和 ibeacon的csv文件
            csv = pd.read_csv(file_path)
            #ibeacon_csv = pd.read_csv(ibeacon_file_path)
            # 设置 参考点标签referencePoint_tag 和 TODO (采样方向标签referencePoint_sample_direction_tag暂未设置）
            samples_dataset = sliceAndAverage(csv, class_tag, "0", column_tags, samples_dataset)
